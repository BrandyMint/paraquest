class AddStateToSlideGame < ActiveRecord::Migration[5.2]
  ...
  def change
    ...
    SlideGame.find_each do |slide_game|
      slide_game.update_columns bundle_game_id: find_or_create_bundle(slide_game).id, state: :done
    end
  ...
  def find_or_create_bundle(slide_game)
    ...
    bundle_game = BundleGame.new state: :draft, user_id: slide_game.user_id, bundle_id: bundle.id
